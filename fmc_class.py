...
class fmc (object):
    ...
    # put 
    def put_acp_rule(self, policy_id, ace_rule_id, data):
        path = "/api/fmc_config/v1/domain/" + self.uuid + "/policy/accesspolicies/"+policy_id+"/accessrules/"+ace_rule_id+"?partialUpdate=true"
        server = "https://"+self.host
        url = server + path
        
        try:
            r = requests.put(url, headers=self.headers, data=json.dumps(data), verify=False)
            #r = requests.put(url, headers=self.headers, data=json.dumps({}), verify=False)
            status_code = r.status_code
            resp = r.text
            json_response = json.loads(resp)
    ...
    # egt 
    def get_acp_rule(self, policy_id, ace_rule_id):
        path = "/api/fmc_config/v1/domain/" + self.uuid + "/policy/accesspolicies/"+policy_id+"/accessrules/"+ace_rule_id
        server = "https://"+self.host
        url = server + path
        try:
            r = requests.get(url, headers=self.headers, verify=False)
    ...
    def get_realms(self):
        ...
        path = "/api/fmc_config/v1/domain/"+ self.uuid + "/object/realms" 
        server = "https://"+self.host
        url = server + path
        url = url + "?filter=realmType:AD,LDAP&expanded=true"
    ...
    def get_realm(self, realm_id):
        ...
        path = "/api/fmc_config/v1/domain/"+ self.uuid + "/object/realms/" + realm_id 
        server = "https://"+self.host
        url = server + path 
        more_items = []
    ...
    # GET Get Groups from Realms 
    def get_groups_from_realm(self, target_domain_id):
        path = "/api/fmc_config/v1/domain/"+ self.uuid + "/object/realmusergroups/" +target_domain_id
        server = "https://"+self.host
        url = server + path 
        more_items = []
    ...
    # GET Get Groups from Realms 
    def get_realmusergroups(self):
        path = "/api/fmc_config/v1/domain/"+ self.uuid + "/object/realmusergroups"
        server = "https://"+self.host
        url = server + path 
        more_items = []
